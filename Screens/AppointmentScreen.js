...
export default function AppointmentScreen({ route, navigation }) {
  ...
  const parseTime = (timeStr) => {
    ...
    const clean = timeStr.replace(/AM|PM/, "").trim();
  ...
  const parseAvailability = (availabilityStr) => {
    const firstCommaIndex = availabilityStr.indexOf(",");
    if (firstCommaIndex === -1) throw new Error("Invalid availability format");

    const daysPart = availabilityStr.substring(0, firstCommaIndex).trim();
    const timePart = availabilityStr.substring(firstCommaIndex + 1).trim();
    ...
    if (daysPart.includes("-")) {
      ...
      allowedDays = [daysRange[daysPart]];
    }
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <View style={{ width: 24 }} /> {/* For centering title */}
      </View>
